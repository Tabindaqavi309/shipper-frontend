...
const onDownload = (action: number) => {
  ...
  if (action == 1) {
    ...
    link.href = "../Authorization to Prepare Electronic Export Information.docx"
    link.click();
  }
  else if (action == 4) {
  ...
  return (
    <Menu secondary style={{ padding: 20 }}>
      ...
      <Dropdown item text='Others'>
        <Dropdown.Menu>
          <Dropdown.Item onClick={() => { onDownload(8) }}>Shipment Checklist</Dropdown.Item>
          <Dropdown.Item onClick={() => { onDownload(9) }}>oti legal disclaime</Dropdown.Item>
          <Dropdown.Item onClick={() => { onDownload(7) }}>values</Dropdown.Item>
          <Dropdown.Item onClick={() => { onDownload(1) }}>AUTHORIZATION TO EXPORT VEHICLE customs</Dropdown.Item>
          <Dropdown.Item onClick={() => { onDownload(2) }}>AUTHORIZATION TO EXPORT VEHICLE own</Dropdown.Item>
          <Dropdown.Item onClick={() => { onDownload(3) }}>Authorization to Prepare Electronic Export Information</Dropdown.Item>
          <Dropdown.Item onClick={() => { onDownload(4) }}>Charleston_Letter_of_Intent</Dropdown.Item>
          <Dropdown.Item onClick={() => { onDownload(5) }}>Drain and Disconnect</Dropdown.Item>
          <Dropdown.Item onClick={() => { onDownload(6) }}>Letterhead</Dropdown.Item>
