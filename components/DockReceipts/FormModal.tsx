...
import { Button, Icon, Modal, TextArea, Form, Loader, Tab, Input } from "semantic-ui-react";
import {
  IDockReceipt_FORM,
...
const FormModal = ({
  ...
  const [effectSection, setEffectSection] = useState<IEffectSection>({} as IEffectSection);
  ...
  const renderBookingView = () => {
    return <BookingReceiptsDropDown formValues={formValues} setFormValues={setFormValues} bookingData={bookingData} />;
  };

  const renderConsigneeView = () => {
    const view = (
      <div>
        <ConsigneeForm formValues={formValues} setFormValues={setFormValues} consigneeData={consigneeData} />
        {renderBookingView()}
      </div>
    );
  ...
  const renderPersonalEffect = () => {
    if (formAction !== "edit") {
      if (isSelected) {
        return createEffectInputsPerContainer(effectSection).map((result, index) => (
            ...
            <div style={{ display: "flex", flexWrap: "wrap" }}>
              {result.type === "text" ? (
                ...
                <Form style={{ marginTop: 10 }}>
                  ...
                  <br />
                  <TextArea placeholder="Personal effect content here" style={{ minHeight: 300, width: 400 }} disabled value={result.value} />
                </Form>
              )}
            </div>
          </div>
        ...
        <Tab.Pane attached={false}>
          <div style={{ height: 1260 }}>
            <div>
              ...
              />
              <CarsComponent carFormValues={carsFormValues} setCarsFormValues={setCarsFormValues} data={data} setData={setData} />
              <div style={{ display: "flex", marginTop: 20 }}>{effectSection.personal_effect !== "" && renderPersonalEffect()}</div>
