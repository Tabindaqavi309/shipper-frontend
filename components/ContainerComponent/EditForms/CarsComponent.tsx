import React, { useState, SetStateAction, Dispatch, SyntheticEvent, ChangeEvent, useRef } from "react";
...
import { ICarFormValues, createCarInputs, carFormValuesObj, IContainerForm, IContainerResponse } from "../../../Types/containerTypes";
...
const CarsComponent = ({ carFormValues, setCarsFormValues, customerId, editFormValues }: IProps) => {
      ...
      createCarInputs(data).map((result, index) => (
        <div style={{ marginRight: 10, marginBottom: 50, width: 300 }} key={index}>
          ...
          <div style={{ display: "flex" }}>
            <Input focus onChange={(e) => handleChange(e, indexOuter)} name={result.name} value={result.value} style={{ width: "100%" }} />
  ...
  const handleAddMoreCars = () => {
    ...
    if (editFormValues.container_type === "roro") {
      if (carFormValues.length >= 1) {
        //error
        setErrorMessage("Operation not possible Roro containers have only one car");
        setError(true);
        setTimeout(() => {
          setError(false);
          setErrorMessage("");
  ...
  const handleVinSearch = () => {
    if (vin !== "") {
      ...
      let canSearch = false;
      for (let i = 0; i < carFormValues.length; i++) {
        if (carFormValues[i].actionType === "add" && carFormValues[i].details === "") {
          canSearch = true;
        }
      }
      ...
      if (canSearch) {
        ...
        searchVIN(vin.toUpperCase()).then((result: any) => {
          if (result.data === "") {
            ...
            for (let i = 0; i < carFormValues.length; i++) {
              if (carFormValues[i].vin === "" || carFormValues[i].details === "") {
                copy[i].details = `${result.data.year} ${result.data.make} ${result.data.model} ${result.data.trim}`;
                copy[i].vin = vin;
              }
            }
        ...
        setErrorMessage("Search not possible when data is present in input field, add a new car or clear the added field and add again.");
        setError(true);
        setTimeout(() => {
          setError(false);
          setErrorMessage("");
  ...
  return (
    <div style={{ marginTop: 20 }}>
      ...
      <div style={{ display: "flex" }}>
        <div>
          <div>
            <Input focus placeholder="Search By VIN..." onChange={(e) => setVin(e.target.value)} value={vin} />
            <Button style={{ marginLeft: 10 }} secondary={vin === "" ? false : true} loading={isSearching ? true : false} onClick={handleVinSearch}>
              Search
            </Button>
          </div>
          ...
          <div>
            <Button style={{ marginTop: 19 }} primary onClick={handleAddMoreCars}>
              Add more cars
            </Button>
          </div>
        </div>
