import React, { useState, SetStateAction, Dispatch, SyntheticEvent, ChangeEvent } from "react";
import { searchVIN } from "../../actions/container";
import { Input, Button, Card, Message, Icon } from "semantic-ui-react";
import { ICarFormValues, createCarInputs, carFormValuesObj, IContainerForm } from "../../Types/containerTypes";
...
const CarsComponent = ({ containerId, carFormValues, setCarFormValues, carData, formValues, setCarData }: IProps) => {
  ...
  const renderCarInput = () =>
    createCarInputs(carFormValues).map((result, index) => (
      <div style={{ marginRight: 10 }} key={index}>
        ...
        <br />
        <Input focus onChange={handleChange} name={result.name} value={result.value} />
      </div>
    ));
  ...
  const renderCarData = () =>
    carData.map((result, index) => (
      <Card style={{ padding: 10 }} key={index}>
        ...
        <p>
          <strong>Value for AES:</strong> {result.value_for_aes === "" ? "-" : result.value_for_aes}
        </p>
      </Card>
    ));
      ...
      <div style={{ display: "flex" }}>
        <div>
          ...
          <div style={{ display: "flex", marginTop: 20 }}>
            {renderCarInput()}
            <div>
              <Button style={{ marginTop: 19 }} primary onClick={handleAddCar} secondary={carFormValues.vin === "" ? false : true}>
                Add
              </Button>
            </div>
          </div>
